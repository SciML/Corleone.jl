...
Constraint wrapper that evaluates a symbolic trajectory getter at selected indices.

# Fields
$(FIELDS)
"""
struct TrajectoryEvaluator{E,G,I,R} <: LuxCore.AbstractLuxLayer
	"The provided symbol or expression"
	name::E
	"The getter for the provided expr"
    getter::G
	"The indices where the trajectory is evaluated"
    idx::I
	"The reducer for the values"
	reducer::R
end

function TrajectoryEvaluator(sys::SymbolCache, grid::AbstractVector, expr, tpoints, reducer = identity)
	getter = getsym(sys, expr)
	idxs = findall(∈(grid), tpoints)
	TrajectoryEvaluator{typeof(expr), typeof(getter), typeof(idxs), typeof(reducer)}(
		expr, getter, idxs, reducer
	)
end

LuxCore.parameterlength(::TrajectoryEvaluator) = 0
LuxCore.statelength(::TrajectoryEvaluator) = 0

function (layer::TrajectoryEvaluator)(traj::Trajectory, ps, st) 
	(; getter, reducer, idx) = layer 
	values = getter(traj)[idx] |> reducer 
	return values, st
end
