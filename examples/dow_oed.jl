...
temperatures = [40.0, 67.0, 100.0] # °C
...
#######################
p_init = [
    1.3978e12, 3.176e12, 4.888e28, 1.847e4, 1.882e4,
    2.636e4, 1.4681e-17, 6.4115e-7, 1.0e-17,
]
...
function dow(du, u, p, t)
    ...
    temperature = p[10]
    k₁ = exp(p[1]) * exp(-p[4] * 1.0e4 / (R) * (1 / temperature - 1 / T₀))
    k₂ = exp(p[2]) * exp(-p[5] * 1.0e4 / (R) * (1 / temperature - 1 / T₀))
    k₋₁ = exp(p[3]) * exp(-p[6] * 1.0e4 / (R) * (1 / temperature - 1 / T₀))
...
probs = map(zip(temperatures, u0s)) do (temperature, u0)
    _prob = DAEProblem{false}(
        dow, du0, u0, tspan, vcat(p_tranformed, temperature),
    ...
    DAEProblem{false}(
        dow, init_dae.du, init_dae.u, tspan, _prob.p,
        initializealg = NoInit(),
        abstol = 1.0e-8, reltol = 1.0e-6,
        sensealg = ForwardDiffSensitivity(),
        differential_vars = [true, true, true, true, true, true, false, false, false, false]
...
sols = [solve(probs[i], DFBDF()) for i in 1:3]
f = Figure()
axs = [CairoMakie.Axis(f[i, 1], title = "Temperature: $(temperatures[i]) K") for i in 1:3]
[plot!(axs[i], sols[i]) for i in 1:3]
f
