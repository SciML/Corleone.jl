...
[lines!(ax, sol.t, reduce(hcat, sol.u)[i, :]) for i in 8:8]
hlines!(ax, [myANC], linestyle = :dash, color = :gray)
ax1 = CairoMakie.Axis(fig[2, 1], title = "Dosage", xlabel = "Time t")
barplot!(ax1, dosetimes[2:end], uopt.u[1:(end - 1)], width = 0.2)
barplot!(ax1, dosetimes2, myu * ones(length(dosetimes2)), width = 0.2, color = :gray)
