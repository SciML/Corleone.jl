...
function make_benchmark(f, Δt, tspans...)
    ...
    loss = let prob = simlayer, dyn = problem, st = st
        function (p, st=st)
            sol, _ = prob(dyn, p, st)
            Xs = map(Array, sol)
            loss = sum(1:2) do i
    ...
    return loss, ComponentVector(ps)
end
...
benchmarks = map([3.0, 1.5, 1.0, 0.5, 0.25, 0.1, 0.05, 0.01]) do dt
    t0 = time()
...
ax = CairoMakie.Axis(f[1,1], xlabel = "|p|", ylabel = "t / s")
#ys = map(x->-x.building, benchmarks)
#scatterlines!(xs, ys, label = "Building Time")
ys = map(x->x.forward.time, benchmarks)
scatterlines!(xs, ys, label = "Forward Simulation")
ys = map(x->x.gradient.time, benchmarks)
scatterlines!(xs, ys, label = "Gradient Computation")
ys = map(x->x.hessian.time, benchmarks)
scatterlines!(xs, ys, label = "Hessian Computation")
display(f)
