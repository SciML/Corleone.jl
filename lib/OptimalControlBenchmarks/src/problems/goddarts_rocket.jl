function goddarts_rocket(grids)
    ...
    @constants begin
        r_0 = 1.0, [tunable = false]
        v_0 = 0.0, [tunable = false]
        m_0 = 1.0, [tunable = false]
        r_T = 1.01, [tunable = false]
        b = 7.0, [tunable = false]
        T_max = 3.5, [tunable = false]
        A = 310.0, [tunable = false]
        k = 500.0, [tunable = false]
        C = 0.6, [tunable = false]
    end
    ...
    grid_cons_u = [310.0 * v(tᵢ)^2 * 2.7182818^(-500.0 * (r(tᵢ) - 1.0)) ≲ 0.6 for tᵢ in constraint_grid]
