...
@testset "Multi-Parameter OED Integration Tests" begin
    ...
    @testset "Two-parameter pharmacokinetics model" begin
        ...
        tspan = (0.0, 10.0)
        k1 = ControlParameter(0.3, (0.1, 1.0), :k1)
        k2 = ControlParameter(0.5, (0.1, 1.0), :k2)
        prob = ODEProblem(pk_model!, u0, tspan, (k1=k1.value, k2=k2.value))
        ...
        oed_layer = create_oed_layer(
            base_layer,
            [:k1, :k2],
            ContinuousMeasurement(k1) => (vars, ps, t) -> vars[1]  # Measure central compartment
        )
        ...
        initial_condition = InitialCondition(u0)
        ...
        @test size(F) == (2, 2)
        @test issymmetric(F)
        @test all(eigvals(F) .>= -1e-10)  # Should be positive semi-definite
    ...
    @testset "Three-parameter SIR epidemic model" begin
        ...
        u0 = [990.0, 10.0, 0.0]
        tspan = (0.0, 50.0)
        β = ControlParameter(0.5, (0.1, 1.0), :β)
        γ = ControlParameter(0.1, (0.01, 0.5), :γ)
        prob = ODEProblem(sir_model!, u0, tspan, (β=β.value, γ=γ.value))
        ...
        oed_layer = create_oed_layer(
            base_layer,
            [:β, :γ],
            ContinuousMeasurement(β) => (vars, ps, t) -> vars[2]  # Measure I(t)
        )
    ...
    @testset "Comparing measurement strategies" begin
        ...
        u0 = [1.0]
        tspan = (0.0, 5.0)
        k1 = ControlParameter(0.5, (0.1, 2.0), :k1)
        k2 = ControlParameter(1.0, (0.5, 2.0), :k2)
        prob = ODEProblem(nonlinear_decay!, u0, tspan, (k1=k1.value, k2=k2.value))
        ...
        oed_cont = create_oed_layer(
            base_layer,
            [:k1, :k2],
            ContinuousMeasurement(k1) => (vars, ps, t) -> vars[1]
        )
        
        rng = Random.default_rng()
        ps, st = LuxCore.setup(rng, oed_cont)
        ic = InitialCondition(u0)
        traj1, st1 = oed_cont(ic, ps, st)
        ...
        add_observed!(sys2, DiscreteMeasurement(k1) => (vars, ps, t) -> vars[1])
        aug_layer2 = SingleShootingLayer(sys2, base_layer)
        oed_disc = OEDLayerV2(sys2, aug_layer2)
        
        ps2, st2 = LuxCore.setup(rng, oed_disc)
        traj2, st2 = oed_disc(ic, ps2, st2)
        meas_times = collect(0.0:0.5:5.0)
        F_disc = discrete_fisher_information(oed_disc, traj2, meas_times)
        
        # Strategy 3: Combined
        sys3 = get_symbolic_equations(base_layer)
        append_sensitivity!(sys3, [:k1, :k2])
        add_observed!(sys3,
            DiscreteMeasurement(k1) => (vars, ps, t) -> vars[1],
            ContinuousMeasurement(k1) => (vars, ps, t) -> vars[1]
        )
        ...
        traj3, st3 = oed_comb(ic, ps3, st3)
        F_comb_cont = fisher_information(oed_comb, traj3)
        F_comb_disc = discrete_fisher_information(oed_comb, traj3, meas_times)
        F_comb_total = F_comb_cont + F_comb_disc
        
        println("\nMeasurement strategy comparison:")
        println("Continuous only: ", tr(F_cont))
        println("Discrete only: ", tr(F_disc))
        println("Combined: ", tr(F_comb_total))
        
        # Combined should give more information
        @test tr(F_comb_total) >= tr(F_cont) - 1e-6
        @test tr(F_comb_total) >= tr(F_disc) - 1e-6
        
        # All should be positive semi-definite
        @test all(eigvals(F_cont) .>= -1e-10)
        @test all(eigvals(F_disc) .>= -1e-10)
        @test all(eigvals(F_comb_total) .>= -1e-10)
    end
end
