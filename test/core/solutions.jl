...
@testset "ControlSegment" begin
    prob = LotkaVolterra.generate()
    sol = solve(prob, Tsit5(), p = [2.0, 1.0, 3.0, 4.0, 0.0, 5.0])
    ...
    @test parameter_values(seg) == [2.0, 1.0, 3.0, 4.0]
    @test current_time(seg) == current_time(sol)
end

@testset "ShootingSegment" begin
    prob = LotkaVolterra.generate()
    sol1 = solve(prob, Tsit5(), p = [2.0, 1.0, 3.0, 4.0, 0.0, 5.0], tspan = (0.0, 5.0))
    ...
    @test parameter_values(seg_cont) == [2.0, 1.0, 3.0, 4.0]
    @test Solutions.control_values(seg_cont) == [[0.0, 5.0], [0.0, 5.0]]
...
@testset "Trajectory" begin
    prob = LotkaVolterra.generate()
    sys = ControlSymbolCache(prob, [:u1, :u2], [:L])
    sol1 = solve(prob, Tsit5(), saveat = 0.2, p = [2.0, 1.0, 3.0, 4.0, 1.0, 0.0], tspan = (0.0, 5.0))
    sol2 = solve(prob, Tsit5(), saveat = 0.2, p = [2.0, 1.0, 2.0, 4.0, 0.0, 1.0], tspan = (5.0, 12.0))
    ...
    @test state_values(traj) == vcat(
        ...
        vcat.(map(Base.Fix1(+, [0.0, 0.0, sol1.u[end][3]]), sol2.u), fill([0.0, 1.0], length(sol2.t)))
    )
end
...
@testset "Trajectory accessors" begin
    prob = LotkaVolterra.generate()
    sys = ControlSymbolCache(prob, [:u1, :u2], [:L])
    sol1 = solve(prob, Tsit5(), saveat = 0.5, p = [2.0, 1.0, 3.0, 4.0, 1.0, 0.0], tspan = (0.0, 5.0))
    sol2 = solve(prob, Tsit5(), saveat = 0.5, p = [2.0, 1.0, 2.0, 4.0, 0.0, 1.0], tspan = (5.0, 12.0))
    seg1 = Corleone.Solutions.ShootingSegment((Solutions.ControlSegment(sol1, sys),), sys)
    seg2 = Corleone.Solutions.ShootingSegment((Solutions.ControlSegment(sol2, sys),), sys)
    traj = Trajectory((seg1, seg2), sys)
