...
@testset "ControlCache" begin
    @testset "Predefined system" begin
        ...
        cache_num = ControlSymbolCache(prob, [3], [3])
        cache_mixed = ControlSymbolCache(prob, [:u1], [3])
        cache_sym = ControlSymbolCache(prob, [:u1], [:L])
        ...
        @test length(cache_mixed.controls) == 1
...
@testset "ControlSegment" begin
    prob = LotkaVolterra.generate()
    sol = solve(prob, Tsit5(), p = [2.0, 1.0, 5.0])
    sys = ControlSymbolCache(prob, [:u1], [:L])
    seg = Corleone.Solutions.ControlSegment(sol, sys)
    @test symbolic_container(seg) == sys
    @test state_values(seg) == vcat.(sol.u, fill([5.0], length(sol.t)))
    @test Solutions.minimal_state_values(seg) == map(Base.Fix2(getindex, 1:2), sol.u)
    @test Solutions.control_values(seg) == [5.0]
    @test parameter_values(seg) == [2.0, 1.0]
    @test current_time(seg) == current_time(sol)
end

@testset "ShootingSegment" begin
    prob = LotkaVolterra.generate()
    sol1 = solve(prob, Tsit5(), p = [2.0, 1.0, 5.0], tspan = (0.0, 5.0))
    sol2 = solve(sol1.prob, Tsit5(), u0 = sol1.u[end], tspan = (5.0, 12.0))
    sys = ControlSymbolCache(prob, [:u1], [:L])
    ...
    @test parameter_values(seg_cont) == [2.0, 1.0]
    @test Solutions.control_values(seg_cont) == [[5.0], [5.0]]
    @test state_values(seg_cont) == vcat(vcat.(sol1.u[1:(end - 1)], fill([5.0], length(sol1.u) - 1)), vcat.(sol2.u, fill([5.0], length(sol2.u))))
end

@testset "Trajectory" begin
    prob = LotkaVolterra.generate()
    sys = ControlSymbolCache(prob, [:u1], [:L])
    sol1 = solve(prob, Tsit5(), saveat = 0.2, p = [2.0, 1.0, 1.0], tspan = (0.0, 5.0))
    sol2 = solve(prob, Tsit5(), saveat = 0.2, p = [2.0, 1.0, 0.0], tspan = (5.0, 12.0))
    ...
    @test state_values(traj) == vcat(
        vcat.(sol1.u[1:(end - 1)], fill([1.0], length(sol1.t) - 1)),
        vcat.(map(Base.Fix1(+, [0.0, 0.0, sol1.u[end][3]]), sol2.u), fill([0.0], length(sol2.t)))
    )
end
...
@testset "Trajectory accessors" begin
    prob = LotkaVolterra.generate()
    sys = ControlSymbolCache(prob, [:u1], [:L])
    sol1 = solve(prob, Tsit5(), saveat = 0.5, p = [2.0, 1.0, 1.0], tspan = (0.0, 5.0))
    sol2 = solve(prob, Tsit5(), saveat = 0.5, p = [2.0, 1.0, 0.0], tspan = (5.0, 12.0))
    seg1 = Corleone.Solutions.ShootingSegment((Solutions.ControlSegment(sol1, sys),), sys)
    seg2 = Corleone.Solutions.ShootingSegment((Solutions.ControlSegment(sol2, sys),), sys)
    traj = Trajectory((seg1, seg2), sys)
...
@testset "ControlSymbolCache SII passthroughs" begin
    prob = LotkaVolterra.generate()
    cache = ControlSymbolCache(prob, [:u1], [:L])
