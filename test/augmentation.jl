...
@testset "Predictions" begin
    ...
    @test isapprox(sol[end], sol_wo_c[end][1:2], atol=1e-7)
    @test isapprox(sol[end], sol_w_c[end][1:2] , atol=1e-7)
    @test isapprox(sol_w_c[end][3:5], sol_wo_c[end][3:5], atol=1e-7)
end
...
@testset "Fisher variables and symmetric_from_vector" begin
    ...
    @test length(f_sym_wo_c) == 0 # All fixed -> this is covered by Corleone.observed_sensitivity_product_variables
